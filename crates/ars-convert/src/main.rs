...
#[clap(
    name = "ars-convert",
    about = "Convert rule lists to .ars binary format",
    version
)]
struct Cli {
    #[clap(subcommand)]
...
fn cmd_convert(
    ...
    println!(
        "✓ {} rules → {}",
        meta.rule_counts.total(),
        output.display()
    );
    println!(
        "  block: exact={} suffix={} keyword={} regex={}",
        meta.rule_counts.block_exact,
        meta.rule_counts.block_suffix,
        meta.rule_counts.block_keyword,
        meta.rule_counts.block_regex
    );
    println!(
        "  allow: exact={} suffix={}",
...
fn cmd_info(file: PathBuf) -> Result<()> {
    ...
    println!(
        "  Allow  exact={} suffix={} keyword={} regex={}",
