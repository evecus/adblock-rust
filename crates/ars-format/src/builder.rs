...
impl ArsBuilder {
    ...
    pub fn set_metadata(&mut self, meta: ArsMetadata) {
        self.metadata = Some(meta);
    }

    fn dedup(&mut self) {
        self.rules.sort_by(|a, b| {
            rule_sort_key(&a.rule_type)
                .cmp(&rule_sort_key(&b.rule_type))
    ...
    pub fn build<W: Write>(&mut self, writer: &mut W) -> Result<ArsMetadata> {
        ...
        for rule in &self.rules {
            match (&rule.action, &rule.rule_type) {
                (RuleAction::Block, RuleType::DomainExact) => {
                    block_exact.push(rule.pattern.clone())
                }
                (RuleAction::Block, RuleType::DomainSuffix) => {
                    block_suffix.push(rule.pattern.clone())
                }
                (RuleAction::Block, RuleType::DomainKeyword) => {
                    block_keyword.push(rule.pattern.clone())
                }
                (RuleAction::Block, RuleType::Regex) => block_regex.push(rule.pattern.clone()),
                (RuleAction::Allow, RuleType::DomainExact) => {
                    allow_exact.push(rule.pattern.clone())
                }
                (RuleAction::Allow, RuleType::DomainSuffix) => {
                    allow_suffix.push(rule.pattern.clone())
                }
                (RuleAction::Allow, RuleType::DomainKeyword) => {
                    allow_keyword.push(rule.pattern.clone())
                }
        ...
        let counts = RuleCounts {
            block_exact: block_exact.len(),
            block_suffix: block_suffix.len(),
            block_keyword: block_keyword.len(),
            block_regex: block_regex.len(),
            allow_exact: allow_exact.len(),
            allow_suffix: allow_suffix.len(),
            allow_keyword: allow_keyword.len(),
            allow_regex: allow_regex.len(),
        ...
        if !block_exact.is_empty() {
            sections.push((SectionId::BlockExact, build_fst(&block_exact)?));
        }
        if !block_suffix.is_empty() {
            sections.push((SectionId::BlockSuffix, build_fst_reversed(&block_suffix)?));
        }
        if !allow_exact.is_empty() {
            sections.push((SectionId::AllowExact, build_fst(&allow_exact)?));
        }
        if !allow_suffix.is_empty() {
            sections.push((SectionId::AllowSuffix, build_fst_reversed(&allow_suffix)?));
        }
        if !block_keyword.is_empty() {
            sections.push((
                SectionId::BlockKeyword,
                block_keyword.join("\n").into_bytes(),
            ));
        }
        if !allow_keyword.is_empty() {
            sections.push((
                SectionId::AllowKeyword,
                allow_keyword.join("\n").into_bytes(),
            ));
        }
        if !block_regex.is_empty() {
            sections.push((SectionId::BlockRegex, block_regex.join("\n").into_bytes()));
        }
        if !allow_regex.is_empty() {
            sections.push((SectionId::AllowRegex, allow_regex.join("\n").into_bytes()));
        }
        if !rewrites.is_empty() {
            sections.push((SectionId::Rewrite, serde_json::to_vec(&rewrites)?));
        }
